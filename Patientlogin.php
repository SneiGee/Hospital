...
if (logged_in()) {
    header("location: patient_home.php?Patient/you-haven't-logged-out");
}
...
if (isset($_POST['patientLogin'])) {
    ...
    if ($email && $password) {
        if (paientEmailExist($email) === TRUE) {
            $Patientlogin = patient_login($email, $password);
            if ($Patientlogin) {
                $patientdata = patientdata($email);

                $_SESSION['id'] = $patientdata['id'];

                header("location: patient_home.php?Pateint/Sigin-Success/Welcome-ID=$patientdata[id]");
            } else {
    ...
    <div class="container" id="allAdminLoginPage">
        ...
        <div class="administrator-loginPage">
            <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post" class="form-horizontal" role="form">
                ...
                <div class="form-group">
                    <div class="col-lg-12">
                        <button type="submit" name="patientLogin" class="btn btn-primary form-control">Sign In</button>
                    </div>
                </div>
                <br>
            </form><!--  end form -->
