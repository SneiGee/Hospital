...
# ==============================================
  ...
  $request = $_REQUEST;
    ...
    while ($row = mysqli_fetch_array($query)) {
        ...
        $subdata[]=$row[7]; // sex
        $subdata[]=$row[5]; // phone
        $subdata[]=$row[10]; // birth DAte    event on button edit in cell database for display
        $subdata[]='<button type="button" id="getEdit" class="btn btn-primary btn-sm" data-toggle="modal" data-target="#myModalEdit" data-id="'.$row[0].'"><i class="fa fa-pencil">&nbsp;</i>Edit</button>
            <a href="doctor_patient.php?delete='.$row[0].'" onclick="return confirm(\'Are you sure you want to delete this data?\')" class="btn btn-danger btn-sm"><i class="fa fa-trash">&nbsp;</i>Delete</button>';
        $data[]=$subdata;
    }
