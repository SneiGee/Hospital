<?php
    ...
    if (isset($_POST['accountantLogin'])) {
        ...
        if (empty($email) || empty($password)) {
            ...
            $query = "SELECT * FROM accountant WHERE email ='$email' AND password='$password'";
            $result = mysqli_query($conn, $query);

            if (mysqli_num_rows($result) == 1) {
                # log administrator to administrator_home page
                $_SESSION['email'] = $email;
                header('location: accountant_home.php?Accountant-SignIn Success Welcome');
            } else {
    ...
    if (isset($_POST['uploadProfile'])) {
        ...
        $query = mysqli_query($conn, "UPDATE accountant SET profile = '".$_FILES['file']['name']."' WHERE email = '".$_SESSION['email']."'");
    }
