<?php
    ...
    if (isset($_POST['laboratoryLogin'])) {
        ...
        if (empty($email) || empty($password)) {
            ...
            $query = "SELECT * FROM laboratory WHERE email ='$email' AND password='$password'";
            $result = mysqli_query($conn, $query);

            if (mysqli_num_rows($result) == 1) {
                # log administrator to administrator_home page
                $_SESSION['email'] = $email;
                header('location: laboratory_home.php?SignIn-Success Welcome');
            } else {
    ...
    if (isset($_POST['uploadProfile'])) {
        ...
        $query = mysqli_query($conn, "UPDATE laboratory SET profile = '".$_FILES['file']['name']."' WHERE email = '".$_SESSION['email']."'");
    }
