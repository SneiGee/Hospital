...
# ==============================================
  ...
  $request = $_REQUEST;
    ...
    while ($row = mysqli_fetch_array($query)) {
        ...
        $subdata[]=$row[3]; // allotment_time
        $subdata[]=$row[4]; // dischargeTime    event on button edit in cell database for display
        $subdata[]='<button type="button" id="getEdit" class="btn btn-primary btn-sm" data-toggle="modal" data-target="#myModalEdit" data-id="'.$row[0].'"><i class="fa fa-pencil">&nbsp;</i>Edit</button>
            <a href="nurse_manageBedAllotment.php?delete='.$row[0].'" onclick="return confirm(\'Are you sure you want to delete this data?\')" class="btn btn-danger btn-sm"><i class="fa fa-trash">&nbsp;</i>Delete</a>';
        $data[]=$subdata;
    }
