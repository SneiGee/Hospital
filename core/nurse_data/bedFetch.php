...
# ==============================================
  ...
  $request = $_REQUEST;
    ...
    $sql = "SELECT * FROM bed WHERE 1=1";
    // Search
    if (!empty($request['search']['value'])) {
        $sql.=" AND (bed_number Like '".$request['search']['value']."%' ";
        $sql.=" OR bed_type Like '".$request['search']['value']."%' )";
    }
    $query = mysqli_query($conn, $sql);
    ...
    while ($row = mysqli_fetch_array($query)) {
        ...
        $subdata[]=$row[3]; // description    event on button edit in cell database for display
        $subdata[]='<button type="button" id="getEdit" class="btn btn-primary btn-sm" data-toggle="modal" data-target="#myModalEdit" data-id="'.$row[0].'"><i class="fa fa-pencil">&nbsp;</i>Edit</button>
            <a href="nurse_manageBed.php?delete='.$row[0].'" onclick="return confirm(\'Are you sure you want to delete this data?\')" class="btn btn-danger btn-sm"><i class="fa fa-trash">&nbsp;</i>Delete</button>';
        $data[]=$subdata;
    }
