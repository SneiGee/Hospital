<?php
    ...
    if (isset($_POST['nurseLogin'])) {
        ...
        if (empty($email) || empty($password)) {
            ...
            # if there's no error allow doctor to log in
            $password = md5($password);//encrypt password before commparing to database
            $query = "SELECT * FROM nurses WHERE email ='$email' AND password='$password'";
            $result = mysqli_query($conn, $query);

            if (mysqli_num_rows($result) == 1) {
                # log administrator to administrator_home page
                $_SESSION['email'] = $email;
                header('location: nurse_home.php?SignIn-Success Welcome');
            } else {
    ...
    if (isset($_POST['uploadProfile'])) {
        ...
        $query = mysqli_query($conn, "UPDATE nurses SET profile = '".$_FILES['file']['name']."' WHERE email = '".$_SESSION['email']."'");
    }
