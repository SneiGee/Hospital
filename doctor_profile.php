...
<?php
    ...
    if (isset($_POST['updateProfileInfo'])) {// update doctor
        ...
        if ($name && $email) {
            $doctorExist = if_doctor_exist($_SESSION['id'], $email);
            if ($doctorExist) {
                $_SESSION['updateError'] = "<div class='updateError'>Oops! Email already exist!</div>";
            } else {
                if (updateDoctorUserData($_SESSION['id']) === TRUE) {
    ...
    if (isset($_POST['changepassword'])) { //changePassword
        ...
        if ($oldPasssword && $newPasssword && $confirmPasssword) {
            if (doctorPasswordMatch($_SESSION['id'], $oldPasssword) === TRUE) {
                if ($newPasssword != $confirmPasssword) {
                    ...
                    if (changeDoctorPassword($_SESSION['id'], $newPasssword) === TRUE) {
        ...
        if ($profile) {
            if (preg_match("!image!", $_FILES['profile']['type'])) {
                if (copy($_FILES['profile']['tmp_name'], $profile)) {
                    if (uploadDoctorProfile($_SESSION['id']) === TRUE) {
                        header("refresh:1; doctor_profile.php");
...
<head>    
    ...
    <title>Profile|myBlogsite Hospital</title>
                ...
                <li class="myBlogsiteDashboard_profile">
                    <?php
                        if ($doctordata['profile'] == "") {
                            echo "<span class='dashboardProfileDefault' data-toggle='modal' data-target='#modal-1'><img width='53' height='53' id='defaultProfile' src='uploads/doctorDefault.jpg' alt='Default Profile'></span>";
                        } else {
                            echo "<span class='dashboardProfileDefault' data-toggle='modal' data-target='#modal-1'><img width='53' height='53' id='defaultProfile' src='uploads/".$doctordata['profile']."' alt='Upload Profile'></span>";
                        }
                ...
                <div class="tab-pane active fade in" id="departmentList">
                    ...
                    <div class="row editProfile">
                        <form>
                            <div class="form-group">
                                <?php
                                    if ($doctordata['profile'] == "") {
                                        echo "<span><img class='editProfilePhoto' data-toggle='modal' data-target='#modal-1' width='230' height='230' src='uploads/doctorDefault.jpg' alt='Default Profile'></span>";
                                    } else {
                                        echo "<span><img class='editProfilePhoto' data-toggle='modal' data-target='#modal-1' width='230' height='230' src='uploads/".$doctordata['profile']."' alt='Profile Photo'></span>";
                                    }
                                ?>
                            </div>
                        </form>
                    ...
                    <div class="row editProfileInfo">
                        <form action='<?php echo $_SERVER['PHP_SELF']; ?>' method='post' class='form-horizontal editProfileInfoForm'>
                            ...
                            <div class='form-group'>
                                <div class='col-lg-6'>
                                    <input type='text' class='form-control' name='doctor_address' value="<?php if (isset($_POST['updateProfileInfo'])) {
                                       echo $_POST['doctor_address'];
                                    } else {
                                       echo $doctordata['doctor_address'];
                                    }?>">
                                </div>
                            </div>
                            <div class='form-group'>
