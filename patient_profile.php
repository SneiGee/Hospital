...
<?php
    ...
    if (isset($_POST['updateProfileInfo'])) {// update patient
        ...
        if ($name && $email) {
            $patientExist = if_patient_exist($_SESSION['id'], $email);
            if ($patientExist) {
                $_SESSION['updateError'] = "<div class='updateError'>Oops! Email already exist!</div>";
            } else {
                if (updatePatientUserData($_SESSION['id']) === TRUE) {
    ...
    if (isset($_POST['changepassword'])) { //changePassword
        ...
        if ($oldPasssword && $newPasssword && $confirmPasssword) {
            if (patientPasswordMatch($_SESSION['id'], $oldPasssword) === TRUE) {
                if ($newPasssword != $confirmPasssword) {
                    ...
                    if (changePatientPassword($_SESSION['id'], $newPasssword) === TRUE) {
        ...
        if ($profile) {
            if (preg_match("!image!", $_FILES['profile']['type'])) {
                if (copy($_FILES['profile']['tmp_name'], $profile)) {
                    if (uploadPatientProfile($_SESSION['id']) === TRUE) {
                        header("refresh:1; patient_profile.php");
...
<head>    
    ...
    <title>Edit Profile | myBlogsite Hospital</title>
                            ...
                            <div class="form-group">
                                <?php
                                    if ($patientdata['profile'] == "") {
                                        ...
                                        echo "<span><img class='editProfilePhoto' data-toggle='modal' data-target='#modal-1' width='230' height='230' src='uploads/".$patientdata['profile']."' alt='Profile Photo'></span>";
                                    }
                                ?>
                            </div>
                        </form>
