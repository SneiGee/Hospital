...
if (logged_in()) {
    header("location: pharmacist_home.php?Phamacist/you-haven't-logged-out");
}
...
if (isset($_POST['pharmacistLogin'])) {
    ...
    if ($email && $password) {
        if (pharmacistEmailExist($email) === TRUE) {
            $Pharmacistlogin = pharmacist_login($email, $password);
            if ($Pharmacistlogin) {
                $pharmacistdata = pharmacistdata($email);

                $_SESSION['id'] = $pharmacistdata['id'];

                header("location: pharmacist_home.php?Phamacist/Sigin-Success/Welcome-ID=$pharmcistdata[id]");
            } else {
        ...
        <div class="administrator-loginPage">
            <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post" class="form-horizontal" role="form">
                <div class="">
                    ...
                    <h3>Sign in</h3>
                    <h5>to continue to Pharmacist</h5>
                </div>
                ...
                <div class="form-group">
                    <div class="col-lg-12">
                        <button type="submit" name="pharmacistLogin" class="btn btn-primary form-control">Sign In</button>
                    </div>
                </div>
                <br>
            </form><!--  end form -->
